...
# Register predict route with explicit debugging
print('Registering /predict route...')
...
def catch_all(path):
    ...
    print(f'Catch-all route triggered for: /{path} with method: {request.method}')
...
if __name__ == '__main__':
    ...
    print('Checking for /predict route...')
    predict_routes = [rule for rule in app.url_map.iter_rules() if 'predict' in rule.rule]
    if predict_routes:
        print(f'✅ /predict route FOUND: {predict_routes[0].rule} [{", ".join(predict_routes[0].methods)}]')
    else:
        print('❌ /predict route NOT FOUND!')
        print('This is a problem - the route should be registered.')
